...
- ``as_dict(instance, *, dict_factory=dict)``: Converts the Data Class
  ...
  are recursed into.  For example::
    ...
    p = Point(10, 20)
    assert as_dict(p) == {'x': 10, 'y': 20}

    c = C([Point(0, 0), Point(10, 4)])
    assert as_dict(c) == {'l': [{'x': 0, 'y': 0}, {'x': 10, 'y': 4}]}

  Raises ``TypeError`` if ``instance`` is not a Data Class instance.

- ``as_tuple(*, tuple_factory=tuple)``: Converts the Data Class
  ...
  Continuing from the previous example::
    ...
    assert as_tuple(p) == (10, 20)
    assert as_tuple(c) == ([(0, 0), (10, 4)],)
