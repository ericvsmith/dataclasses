...
- ``eq``: If true (the default), ``__eq__`` and ``__ne__`` methods
  ...
  the identical type, or one must be a subtype of the other and have
...
- ``order``: If true (the default), ``__lt__``, ``__le__``,
  ...
  instances in the comparison must be of the identical type, or one
  must be a subtype of the other and have no additional fields
  defined.  If ``order`` is true and ``eq`` is false, a ``ValueError``
  is raised.
...
If any generated method already exists on the class, an
...
must always be specified.  Consider the case where a ``context`` or
``database`` object is needed to create an instance, but isn't stored
as a field.  With alternate classmethod constructors the ``context``
or ``database`` parameters are always optional, because you could
still create the object by going through ``__init__``.  Which approach
is more appropriate will be application-specific, but both approaches
are supported.

Instance comparison
-------------------

The goal of the instance comparison logic "both instances in the
comparison must be of the identical type, or one must be a subtype of
the other and have no additional fields defined" is that the following
case will work::
...
That is, you can compare an instance of a class and an instance of a
subclass, as long as no fields have been added to the subclass.  Note
that this only applies to real fields: the subclass can add
``ClassVar`` and ``InitVar`` pseudo-fields and comparisons will still
work.
