...
The ``dataclass`` decorator will generate various "dunder" methods on
the class, described below.  If any generated method already exists on
the class, a ``TypeError`` will be raised.  The decorator returns the
same class that is called on: no new class is created.
