...
- ``hash``: This can be a bool or ``None``.  If True, this field is
  ...
  be if a field is expensive to compute a hash value for, but there
  are other fields that contribute to the type's hash value.  Even if
  a field is excluded from the hash, it can still be compared when
