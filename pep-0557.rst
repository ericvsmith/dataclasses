...
would be copied from the source object, but that was deemed to be
inconsistent with using ``__init__`` and ``__post_init__`` to
...
The post-init function ``__post_init__`` takes no parameters.  This
was deemed to be simpler than trying to find a mechanism to optionally
pass a parameter to the ``__post_init__`` function.
